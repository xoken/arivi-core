...
testMerkleProofN n
  ...
  | otherwise = do
      ...
      let mtree = mkMerkleTree $ map (BSC.pack . show) [1..n]
          randLeaf = mkLeafRootHash $ (BSC.pack . show) randN
          proof = merkleProof mtree randLeaf
      return $ validateMerkleProof proof (mtRoot mtree) randLeaf


instance Serialise Side
instance (Serialise a) => Serialise (ProofElem a)
instance (Serialise a) => Serialise (MerkleProof a)
instance (Serialise a) => Serialise (MerkleNode a)
instance (Serialise a) => Serialise (MerkleRoot a)

...
instance ( Ord t
         ...
         , Eq msg
         , Hashable msg
         , MonadReader (PubSubEnv TVar t msg) m
         , MonadIO m
         ) =>
         HasSubscribers m t msg where
    subscribers t = do
        Subscribers subs <- asks pubSubSubscribers
        case subs ^. at t of
            Just x -> liftIO $ readTVarIO x
            Nothing -> return Set.empty
    subscribers' msg t = do
        Inbox inbox <- join $ liftIO . readTVarIO <$> asks pubSubInbox
...
instance (Eq t, Hashable t, MonadReader (PubSubEnv TVar t msg) m, MonadIO m) =>
         HasNotifiers m t where
    notifiers t = do
        Notifiers notifs <- asks pubSubNotifiers
        case notifs ^. at t of
            Just x -> liftIO $ readTVarIO x
            Nothing -> return Set.empty
    ...
    handleTopic nid t msg = do
        ...
        case cache ^. at msg of
            ...
            Nothing -> do
                ...
                TopicHandlers handlers <- asks pubSubHandlers

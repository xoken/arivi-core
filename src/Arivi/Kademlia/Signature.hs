...
module Arivi.Kademlia.Signature
(
    getSecretKey,
    getPublicKey,
    sign,
...
publicKeytoHex mPublicKey = Data.ByteString.Base16.encode
                                (toByteString mPublicKey)

-- | Converts PublicKey from hex form to PublicKey form
hexToPublicKey :: ByteString -> PublicKey
hexToPublicKey hexPublicKey = Crypto.Error.throwCryptoError
                                (Crypto.PubKey.Ed25519.publicKey
                                (fst (Data.ByteString.Base16.decode
                                hexPublicKey)))


-- | This function generates (SecretKey,PublicKey) pair using Raaz's Random
--   Seed generation
...
secretKeyToHex mSecretKey = Data.ByteString.Base16.encode
                                (Data.ByteArray.convert mSecretKey)::ByteString


-- | This function is used for converting Secret Key from hex form to SecretKey
--   form
hexToSecretKey :: ByteString -> SecretKey
hexToSecretKey hexSecretKey = Crypto.Error.throwCryptoError
                                (Crypto.PubKey.Ed25519.secretKey
                                (fst (Data.ByteString.Base16.decode
                                hexSecretKey)))

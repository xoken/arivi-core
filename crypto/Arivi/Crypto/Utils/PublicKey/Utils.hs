...
import Data.ByteString.Char8 (ByteString, concat, splitAt)
...
getSignaturePublicKeyFromNodeId nodeId = pk
  where
...
getEncryptionPublicKeyFromNodeId nodeId = pk
  where
    pkPair = Data.ByteString.Char8.splitAt 32 nodeId
    pkBs = snd pkPair
...
createSharedSecretKey remotePubKey signSK = finalssk
  where
...
deriveSharedSecretKey remotePubKey signSK = finalssk
  where
    encryptSK = getEncryptionSecretKey signSK
...
generateNodeId signsk = Data.ByteString.Char8.concat [signingPK, encryptionPK]
  where
...
encryptMsg aeadnonce ssk header msg = ciphertext
  where
...
decryptMsg aeadnonce ssk header tag ct = plaintext
  where
    sskBS = Encryption.sharedSecretToByteString ssk
    aeadBS = L.toStrict $ Binary.encode aeadnonce

...
import qualified Arivi.Network.Types as ANT (PersonalityType(INITIATOR), TransportType(TCP))
...
sender sk rk n size = do
  ...
  -- sock <- createUDPSocket "127.0.0.1" (envPort mkAriviEnv)
    mutableConnectionHashMap <- MutableHashMap.new :: IO (HashTable ConnectionId CompleteConnection)
    env' <- mkAriviEnv
    let env =
            env'
                { ariviCryptoEnv = CryptoEnv sk
                , loggerChan = tq
                 -- , udpSocket = sock
...
receiver sk = do
    tq <- newTQueueIO :: IO LogChan
  -- sock <- createUDPSocket "127.0.0.1" (envPort mkAriviEnv)
    mutableConnectionHashMap1 <- MutableHashMap.new :: IO (HashTable ConnectionId CompleteConnection)
    env' <- mkAriviEnv
    let env =
            env'
                { ariviCryptoEnv = CryptoEnv sk
    ...
    runStdoutLoggingT $
        runAppM
            env
            (do runTCPServer (show (envPort env))

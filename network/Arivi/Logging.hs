...
class (MonadLogger m, MonadIO m, MonadBaseControl IO m, MonadThrow m, MonadCatch m) => HasLogging m where
...
withChanLocLogging :: (HasLogging m) => Loc -> LogStatement -> LogLevel -> m a -> m a
...
timeIt :: forall m a e . (MonadIO m, MonadBaseControl IO m, Exception e) => m a -> m (Double, Either e a)
timeIt ioa = do

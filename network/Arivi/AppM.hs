...
sender sk rk = do
  ...
  -- sock <- createUDPSocket "127.0.0.1" (envPort mkAriviEnv)
    mutableConnectionHashMap <-
        MutableHashMap.new :: IO (HashTable ConnectionId Connection)
    env' <- mkAriviEnv
    let env =
            env'
...
receiver sk = do
  ...
  -- sock <- createUDPSocket "127.0.0.1" (envPort mkAriviEnv)
    ...
    runStdoutLoggingT $ runAppM env (runUDPserver (show (envPort env)))

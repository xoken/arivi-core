...
sender sk rk = do
  ...
  runStdoutLoggingT $ runAppM env (do
                                       let ha = "127.0.0.1"
                                       cid <- openConnection ha 8080 ANT.UDP (generateNodeId rk) ANT.INITIATOR
...
writeToEventTChan eventTChannel = forever $ do
    let dataPayload = Payload "hello"
    let dataEvent = SendDataEvent dataPayload
    atomically $ writeTChan eventTChannel dataEvent
...
main = do
 ...
 --     -- runTCPserver (show (envPort env))
                                       ...
                                       -- liftIO $ print cid
                                       -- conn <- lookupCId cid
                                       -- let eventTChannel = eventTChan conn
                                       -- liftIO $ writeToEventTChan eventTChannel
                                       -- liftIO $ print cid
